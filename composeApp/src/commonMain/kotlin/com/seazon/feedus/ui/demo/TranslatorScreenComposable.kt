...
@Composable
fun TranslatorScreenComposable(
    translate: (type: String, key: String, appId: String, query: String) -> Unit,
) {
    ...
    Column(
        ...
        // appId
        if (typeValue.value == TranslatorUtil.TYPE_BAIDU) {
            Spacer(modifier = Modifier.height(8.dp))
            Row(
                verticalAlignment = Alignment.CenterVertically
            ) {
        ...
        FmPrimaryButton(
            ...
            text = "translate",
            onClick = {
                translate(typeValue.value, keyValue.value, appIdValue.value, queryValue.value)
            }
        )
    }
}
...
@Preview
@Composable
fun TranslatorScreenComposablePreview() {
    TranslatorScreenComposable(translate = { type, key, appId, query -> })
}
