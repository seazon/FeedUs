...
object HttpUtils {
    ...
    private val CHARSET_CHARACTER = arrayOf("content=\"text/html;charset=", "content='text/html;charset=", "charset=\"")
...
//                        LogUtils.debug("charset:" + charset)
    ...
    fun format(
        ...
        for (parameter in parameters) {
            val encodedName = encode(parameter.name, encoding)
            val value = parameter.value
            val encodedValue = if (value != null) encode(value, encoding) else ""
    ...
    private fun encode(content: String?, encoding: String?): String? {
        try {
            return URLEncoder.encode(
                content,
                if (encoding != null) encoding else DEFAULT_CHARSET
            )
